...
def full(rnd, depth, func_list, terminal_list):
    ...
    if depth == 0:
        ...
        for _, p in inspect.signature(func).parameters.items():
            if p.default is not inspect.Parameter.empty:
                continue
            node = full(rnd, depth - 1, func_list, terminal_list)
            args.append(node)
        return Node(func, args)

def ramped_half_and_half(rnd, min_depth, max_depth, func_list, terminal_list):
    ''' Generate a population of half full and half grow trees '''
    depth = rnd.randint(min_depth, max_depth+1)
    if rnd.rand() < 0.5:
        return grow(rnd, depth, None, func_list, terminal_list)
    else:
        return full(rnd, depth, func_list, terminal_list)

def select_node(rnd, leaf_prob, in_node: Node, filter, break_filter) -> Optional[Node]:
    places = [(at_d, n) for at_d, n in in_node.traverse(filter, break_filter) ]
    if len(places) == 0:
        return None, None 
    if leaf_prob is None: 
        selected_idx = rnd.choice(len(places))
        ...
        if (rnd.rand() < leaf_prob and len(leaves) > 0) or len(nonleaves) == 0:
...
def subtree_mutation(rnd, leaf_prob, max_depth, func_list, terminal_list, node):
    new_node = grow(rnd, 5, None, func_list, terminal_list)
    new_node_depth = new_node.get_depth()
    # at_depth, at_node = select_node(rnd, leaf_prob, node, lambda d, n: (d > 0) and n.is_of_type(new_node), 
    #                                     lambda d, n: (d + new_node_depth) <= max_depth)
    at_depth, at_node = select_node(rnd, leaf_prob, node, lambda d, n: n.is_of_type(new_node), 
                                        lambda d, n: (d + new_node_depth) <= max_depth)
    if at_node is None:
        return node
...
def subtree_crossover(rnd, leaf_prob, max_depth, parent1: Node, parent2: Node):
    ''' Crossover two trees '''
    if parent1.get_depth() == 0 or parent2.get_depth() == 0:
        return parent1, parent2
    parent1, parent2 = sorted([parent1, parent2], key = lambda x: x.get_depth())
    # for _ in range(3):
    # at1_at_depth, at1 = select_node(rnd, leaf_prob, parent1, lambda d, n: (d > 0), lambda d, n: True)
    at1_at_depth, at1 = select_node(rnd, leaf_prob, parent1, lambda d, n: True, lambda d, n: True)
    if at1 is None:
        return parent1, parent2
    at1_depth = at1.get_depth()
    at2_depth, at2 = select_node(rnd, leaf_prob, parent2, 
...
def subtree_breed(rnd, mutation_rate, crossover_rate, 
